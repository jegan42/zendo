...
async function upsertAddress(req: Request, res: Response) {
    try {
        const userId = (req as any).userId; // Récupéré depuis le middleware d'authentification
        const { phone, street, postalCode, city, country, addressType } =
            req.body;
        ...
        return res
            .status(200)
            .json({ message: `Adresse ${addressType} enregistrée`, address });
    } catch (error) {
