...
async function getCartItems(req: Request, res: Response) {
  try {
    ...
    const user = (await User.findById({ _id: userId }).populate("cart")) as any;
    ...
    // Etape 4 : construire un tableau d'objets simples enrichis avec la variation
    // en conservant la logique de boucle for
    const cartWithVariations = [];
    for (let i = 0; i < (user.cart || []).length; i++) {
      const item = user.cart[i];
      ...
      cartWithVariations.push({
        ...item.toObject(),
        variation: variation,
      });
    }

    // Etape 5 : renvoyer les produits du panier de l'utilisateur
    return res.status(200).json({
      message: "Produits du panier récupérés",
      cart: cartWithVariations,
    });
  } catch (error) {
