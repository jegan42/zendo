...
const UserInfosModal = ({ isOpen, onClose }: UserInfosModalProps) => {
    ...
    useEffect(() => {
        const loadUserData = async () => {
            ...
            // 1. Remplir les infos utilisateur depuis Redux
            if (user) {
            ...
            // 2. Récupérer l'adresse depuis le backend
            try {
                let userId = user?._id;
                if (!userId && token) {
    ...
    const handleSave = async () => {
        try {
            // 1️⃣ Récupération de l'ID utilisateur
            let userId = user?._id;
            if (!userId && token) {
                const decoded: any = jwtDecode(token);
            ...
            // 4️⃣ Exécution des deux requêtes en parallèle
            const [userRes] = await Promise.all([userPromise, addressPromise]);

            // 5️⃣ Mise à jour Redux
            dispatch(
                setCredentials({
